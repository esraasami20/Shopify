...
namespace Shopify.Services
{
    public class ReviewService
    {
        ...
        public Response AddNewReview(IIdentity customer, Review review)
        {
            ...
            var customerId = HelperMethods.GetAuthnticatedUserId(customer);
            var product = _db.Products.Include(r=>r.Reviews).FirstOrDefault(a => a.ProductId == review.ProductId);
            if (product != null)
            {
        ...
        private void UpdateProductReview(int productId){
           Product product = _db.Products.Include(r=>r.Reviews).SingleOrDefault(p => p.ProductId == productId && p.IsdeletedBySeller==false && p.Active==false);
